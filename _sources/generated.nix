# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  agenix = {
    pname = "agenix";
    version = "9edb1787864c4f59ae5074ad498b6272b3ec308d";
    src = fetchFromGitHub {
      owner = "ryantm";
      repo = "agenix";
      rev = "9edb1787864c4f59ae5074ad498b6272b3ec308d";
      fetchSubmodules = false;
      sha256 = "sha256-NA/FT2hVhKDftbHSwVnoRTFhes62+7dxZbxj5Gxvghs=";
    };
    date = "2025-08-05";
  };
  blog = {
    pname = "blog";
    version = "aed7e0780ae6cbaf32e715c5d266cb1adb4a8e02";
    src = fetchgit {
      url = "https://git.gaze.systems/90008/website.git";
      rev = "aed7e0780ae6cbaf32e715c5d266cb1adb4a8e02";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-v04VvwNR85M1lXQIgi7XCwcuByZe0rOIyDz+0vWW1ic=";
    };
    date = "2025-08-19";
  };
  gomod2nix = {
    pname = "gomod2nix";
    version = "7f963246a71626c7fc70b431a315c4388a0c95cf";
    src = fetchFromGitHub {
      owner = "nix-community";
      repo = "gomod2nix";
      rev = "7f963246a71626c7fc70b431a315c4388a0c95cf";
      fetchSubmodules = false;
      sha256 = "sha256-YVoIFDCDpYuU3riaDEJ3xiGdPOtsx4sR5eTzHTytPV8=";
    };
    date = "2025-08-01";
  };
  home = {
    pname = "home";
    version = "f8af2cbe386f9b96dd9efa57ab15a09377f38f4d";
    src = fetchFromGitHub {
      owner = "nix-community";
      repo = "home-manager";
      rev = "f8af2cbe386f9b96dd9efa57ab15a09377f38f4d";
      fetchSubmodules = false;
      sha256 = "sha256-ib1Xi13NEalrFqQAHceRsb+6aIPANFuQq80SS/bY10M=";
    };
    date = "2025-08-17";
  };
  limbusart = {
    pname = "limbusart";
    version = "7f29ab57b73eb8a4db7163fc7d19a71cf89c3d8b";
    src = fetchgit {
      url = "https://git.gaze.systems/90008/limbusart.git";
      rev = "7f29ab57b73eb8a4db7163fc7d19a71cf89c3d8b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-xATZz/sHYHkEXjwcmq1DSntclWxnDIKCpH314MaL63U=";
    };
    date = "2025-07-22";
  };
  lix-module = {
    pname = "lix-module";
    version = "2.93.3-1";
    src = fetchTarball {
      url = "https://git.lix.systems/lix-project/nixos-module/archive/2.93.3-1.tar.gz";
      sha256 = "sha256-KYMUrTV7H/RR5/HRnjV5R3rRIuBXMemyJzTLi50NFTs=";
    };
  };
  ncr = {
    pname = "ncr";
    version = "4660aa9cd9b24bef569696eee46dbe7e8df3e237";
    src = fetchgit {
      url = "https://tangled.sh/@poor.dog/nixos-cloud-resources";
      rev = "4660aa9cd9b24bef569696eee46dbe7e8df3e237";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-miZ64o8p+ToR6VIpcco/wq0rdHGjHsfepJdMcSiJEDk=";
    };
    date = "2025-07-18";
  };
  nixos-wsl = {
    pname = "nixos-wsl";
    version = "203a7b463f307c60026136dd1191d9001c43457f";
    src = fetchFromGitHub {
      owner = "nix-community";
      repo = "NixOS-WSL";
      rev = "203a7b463f307c60026136dd1191d9001c43457f";
      fetchSubmodules = false;
      sha256 = "sha256-EOqCupB5X5WoGVHVcfOZcqy0SbKWNuY3kq+lj1wHdu8=";
    };
    date = "2025-08-15";
  };
  nsid-tracker = {
    pname = "nsid-tracker";
    version = "a4dd7621f5b3a5a3136d7e187caa90ac38c6a402";
    src = fetchgit {
      url = "https://tangled.sh/@poor.dog/nsid-tracker";
      rev = "a4dd7621f5b3a5a3136d7e187caa90ac38c6a402";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-aYoe2uo1VR1PdC2mMbgX4A0rlMt24KrxcMUl7o2WFb8=";
    };
    date = "2025-08-23";
  };
  tangled = {
    pname = "tangled";
    version = "634c03e1c668d83693ff5105f88ad1a57903564a";
    src = fetchgit {
      url = "https://tangled.sh/@tangled.sh/core";
      rev = "634c03e1c668d83693ff5105f88ad1a57903564a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-lEUh0UlR1XDIXPH+20Wj5I+vVbhKLk8pRhPy5fEVysQ=";
    };
    date = "2025-08-23";
  };
  tangled-sqlite-lib = {
    pname = "tangled-sqlite-lib";
    version = "3450100";
    src = fetchTarball {
      url = "https://sqlite.org/2024/sqlite-amalgamation-3450100.zip";
      sha256 = "sha256-bJoMjirsBjm2Qk9KPiy3yV3+8b/POlYe76/FQbciHro=";
    };
  };
}
